...
void multiply(int m, int n, double m1[][n], int p, int q, double m2[][q], double res[][q])
{
    int c, d, k;
    double sum = 0;
    #pragma omp parallel for default(none) private(c, d, k, sum) shared(res, m1, m2,  m, n, p, q)
    for (c = 0; c < m; c++) {
      for (d = 0; d < q; d++) {
        sum = 0;
        for (k = 0; k < p; k++) {
...
void absSumDiag(int n, double A[][n]) {
  int i, j;
  double sum[n];
  //#pragma omp for default(none) shared(i, n, A)
  for(i=0; i<n; i++) {
    sum[i] = 0;
    for(j=0; j<n; j++) {
        sum[i] += fabs(A[j][i]);
...
void copyM(int n, double A[][n], int start, int size, double res[][size]) {
  int i,j;
  #pragma omp parallel for default(none) private(i,j) shared(size, res, A, start) collapse(2)
  for(i=0; i<size; i++) {
    for(j=0; j<size; j++)
...
void assginM(int size, int startX, int startY, double A[][size], int p, int q, double set[][q]) {
  int i,j;
  #pragma omp parallel for default(none) private(i,j) shared(set, A, startX, startY, p, q) collapse(2)
  for(i=0; i<p; i++) {
    for(j=0; j<q; j++)
...
void computerInverse(int n, double A[][n]) {
  ...
  if(n<16) {
    ...
    #pragma omp task default(none) shared(n1, R11) if(n>512)
    computerInverse(n1, R11);

    //printf("start computerInverse \n");
    double sizeR22 = n-n1;
    #pragma omp task default(none) shared(sizeR22, R22) if(n>512)
    ...
    #pragma omp parallel for default(none) private(i,j) shared(R12, n, n1) collapse(2)
    for(i=0; i<n1; i++) {
      for(j=0; j<n-n1; j++) {
        R12[i][j] *= -1.0;
      }
